...
export default function ProductsPage() {
    ...
    const fetchAllProducts = async () => {
        try {
            const response = await axios.get(`http://localhost:8081/api/all-products?page=${currentPage}&size=${productsPerPage}`);
            console.log("API Response:", response.data);
            setProducts(response.data.content || []); // Ensure we always have an array
